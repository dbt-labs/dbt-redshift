...
from dbt.adapters.redshift.relation.models import RedshiftSchemaRelation
...
def test_make_schema(config_dict: dict, exception: Type[Exception]):
    if exception:
        with pytest.raises(exception):
            RedshiftSchemaRelation.from_dict(config_dict)
    else:
        my_schema = RedshiftSchemaRelation.from_dict(config_dict)
